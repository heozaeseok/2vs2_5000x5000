...
class SocketMultiAgentEnv(BaseEnv):
    def __init__(self, cfg):
        ...
        self.num_agents = cfg.get('num_agents', 2)          # 아군 2명
        self.num_detectable = cfg.get('num_detectable', 2)  # 적 2명 감지

        # 팀1(아군) 2명, 팀0(적군) 2명
        self.agent_ids = ["AgentCharacter", "AgentCharacter2"]     # 아군
        self.enemy_ids = ["AgentCharacter3", "AgentCharacter4"]    # 적군
        ...
        self.valid_actions = [0,1,2,3,4,5,6,7,9,10] 
    ...
    def _convert_obs(self, data, center=None, scale_xy: float = 5000, scale_z: float = 500):
        """
        UE 좌표 → (중심점 기준 상대좌표) → 맵 스케일 정규화
        - center: (cx, cy, cz). None이면 기본 중심 사용.
        - scale_xy/scale_z: 정규화 분모(훈련 박스 크기)
        """
        # 중심점 설정
        if center is None:
            cx, cy, cz = 2550.0, 2510.0, 410.0
        else:
            cx, cy, cz = center
        ...
        for other_agent in self.agent_ids:
            ...
            if other_data:
                ...
                obs.extend([
                    ox / scale_xy, oy / scale_xy, oz / scale_z, ohp / 100.0
                ])
            else:
        ...
        for enemy in data.get("DetectActors", []):
            ...
            if name in fixed_enemy_names:
                ...
                obs[base:base+4] = [
                    ex / scale_xy, ey / scale_xy, ez / scale_z, 1.0
                ]  # 마지막 1.0은 존재 표시
    ...
    def _enemy_seek_or_shoot_actions(self, x_target=2620.0, y_target=2410.0, tol=0.0):
        """
